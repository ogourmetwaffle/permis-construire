...
export async function POST(req: Request) {
  try {
    ...
    const { data: dossier, error } = await supabaseAdmin
      .from('dossiers')
      .select('id, numero_dossier, statut, montant, montant_acompte, iban, bic, titulaire, reference_virement, commentaire_admin, commentaire_statut, created_at, updated_at')
